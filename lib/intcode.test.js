...
test('run throws when there is an illegal opcode', (t) => {
  t.throws((() => intcode.run([88])), 'Illegal opcode 88');
});

test('run throws EOP when there is no HALT/99 opcode', (t) => {
  t.throws((() => intcode.run([1, 0, 0, 0])), 'Reached EOP without opcode 99');
});

test('parseOpcode handles regular opcode', (t) => {
  t.deepEqual(intcode.parseOpcode(1, intcode.INSTRUCTIONS), {
    value: 1,
    modes: [0, 0, 0],
  });
});
