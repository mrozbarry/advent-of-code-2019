...
const debug = (
  //(instruction, ...args) => (void console.log(
    //`0x${instruction.address}`, JSON.stringify(instruction.segment),
    //...args
  //))
  () => {}
);
...
const mathOpcode = (sign, mathFn) => makeOpcode(2, 1, (program, instruction) => {
  ...
  debug(instruction, 'math', a, sign, b, '=', result, '=>', `0x${addr}`);
...
const jumpOpcode = (op, conditionFn) => makeOpcode(2, 0, (program, instruction) => {
  const shouldJump = conditionFn(instruction.params[0]);
  const ifTrueAddr = instruction.params[1];
  const ifFalseAddr = program.address + 3;
  const nextAddress = shouldJump ? ifTrueAddr : ifFalseAddr;
  debug(
    instruction,
    'If', instruction.params[0], op, `(${shouldJump.toString()})`,
    'jump to address', ifTrueAddr,
    'else address', ifFalseAddr,
  );
  return {
    ...program,
    address: nextAddress,
...
const boolOpcode = (op, conditionFn) => makeOpcode(2, 1, (program, instruction) => {
  ...
  debug(
    instruction,
    'If', a, op, b, `(${shouldWrite.toString()})`,
    'write 1 to address', addr,
    'else write 0',
  );
  return {
    ...addressWrite(
...
const OPCODES = {
  ...
  [INSTRUCTIONS.INPUT]: makeOpcode(0, 1, async (program, instruction, adapter) => {
    ...
    debug(instruction, 'Got input', input);
    return ({
      ...addressWrite(
        program,
  ...
  [INSTRUCTIONS.OUTPUT]: makeOpcode(1, 0, (program, instruction, adapter) => {
    adapter.output(instruction.params[0]);
    debug(instruction, 'Will output', instruction.params[0]);
    return {
      ...program,
      address: program.address + 2,
    };
...
const step = (program, adapter) => {
  ...
  // console.log('intcode.instruction', instruction);
